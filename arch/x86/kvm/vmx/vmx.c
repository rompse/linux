...
#define INT_PAGE_SIZE_4K                    4096u
#define INT_PAGE_OFFSET_MASK_4K             (INT_PAGE_SIZE_4K - 1)
#define INT_PAGE_MAX_OFFSET_4K              INT_PAGE_OFFSET_MASK_4K
#define INT_PAGE_BASE_MASK_4K               (0xFFFFFFFFFFFFFFFFULL - INT_PAGE_MAX_OFFSET_4K)
#define INT_PAGE_REMAINING(addr)            (INT_PAGE_SIZE_4K - ((addr) & INT_PAGE_OFFSET_MASK_4K))
#define INT_PAGE_OFFSET_4K(addr)            ((addr) & INT_PAGE_OFFSET_MASK_4K)
#define INT_PAGE_FRAME_NUMBER_4K(addr)      ((addr) >> 12ULL)
#define INT_PAGE_COUNT_4K(addr, bytes)      (INT_PAGE_FRAME_NUMBER_4K(INT_PAGE_OFFSET_4K(addr) + (bytes) + INT_PAGE_MAX_OFFSET_4K))
#define INT_PAGE_FRAME_NUMBER(addr)         INT_PAGE_FRAME_NUMBER_4K(addr)
#define INT_PAGE_COUNT(addr, bytes)         INT_PAGE_COUNT_4K(addr, bytes)

#define INT_TRACE(str) printk(KERN_ALERT "[%s] " str " | %s:%i\n", __PRETTY_FUNCTION__, __FILE__, __LINE__)
...
void phys_read_or_write(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer, bool write) {
	struct kvm_host_map map;

	if (!vcpu || !addr || !len || !buffer) {
		INT_TRACE("INVALID ARGS");
		return;
	}

	if (INT_PAGE_COUNT(addr, len) > 1) {
		INT_TRACE("INT_PAGE_COUNT > 1");
		return;
	}

	if (kvm_vcpu_map(vcpu, gpa_to_gfn(addr), &map)) {
		INT_TRACE("VCPU_MAP FAILURE");
		return;
	}

	if (!map.hva) {
		INT_TRACE("MAPPED PAGE HVA INVALID");
		return;
	}

	if (write) {
		memcpy(map.hva, buffer, len);
	} else {
		memcpy(buffer, map.hva, len);
	}

	kvm_vcpu_unmap(vcpu, &map, true);
}

void phys_read(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer)
{
	return phys_read_or_write(vcpu, addr, len, buffer, false);
}

void phys_write(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer)
{
	return phys_read_or_write(vcpu, addr, len, buffer, true);
}

u64 virt_map(struct kvm_vcpu* vcpu, u64 addr, struct kvm_host_map* map, bool kernel) {
	u64 gpa = kernel ? kvm_mmu_gva_to_gpa_system(vcpu, addr, NULL) :
			   kvm_mmu_gva_to_gpa_write(vcpu, addr, NULL);
	if (!gpa) {
		INT_TRACE("INVALID GPA");
		return 0;
	}

	if (kvm_vcpu_map(vcpu, gpa_to_gfn(gpa), map)) {
		printk(KERN_ALERT "VCPU_MAP: 0x%llx -> 0x%llx\n", addr, gpa);
		INT_TRACE("VCPU_MAP FAILURE");
		return 0;
	}
	...
	return map->hva + offset_in_page(gpa);
}

void virt_unmap(struct kvm_vcpu* vcpu, struct kvm_host_map* map) {
	kvm_vcpu_unmap(vcpu, map, true);
}

void virt_read_or_write(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer, bool write, bool kernel) {
	u32 left = len;
	u64 gva = addr;
	u8* buf = buffer;
	...
	do
	{
		...
		u32 size = min(left, INT_PAGE_REMAINING(gva));

		u64 kaddr = virt_map(vcpu, addr, &map, kernel);
		if (!kaddr) {
			INT_TRACE("INVALID KADDR");
			return;
		}

		if (write) {
			memcpy(kaddr, buf, size);
		}
		else {
...
void virt_read(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer) {
	virt_read_or_write(vcpu, addr, len, buffer, false, false);
}

void virt_write(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer) {
	virt_read_or_write(vcpu, addr, len, buffer, true, false);
}

void virt_read_kernel(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer) {
	virt_read_or_write(vcpu, addr, len, buffer, false, true);
}

void virt_write_kernel(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer) {
	virt_read_or_write(vcpu, addr, len, buffer, true, true);
}

u64 get_kernel_base(struct kvm_vcpu* vcpu) {
	struct msr_data msr;
	msr.index = 0xC0000082;
	msr.host_initiated = true;

	vmx_get_msr(vcpu, &msr);

	u64 entry = ((u64)msr.data) & ~0xfff;
	do {
		if (!entry) {
			INT_TRACE("ENTRY EMPTY...");
			break;
		}

		u16 addr;
		virt_read_kernel(vcpu, entry, sizeof(addr), &addr);
		if (addr == 0x5A4D) {
			u64 x = 0;
			for (x = entry; x < entry + 0x400; x += 8) {
				u64 out;
				virt_read_kernel(vcpu, x, sizeof(out), &out);
				if (out == 0x4B4C45474150) {
					return entry;
				}
			}
		}
	...
	INT_TRACE("FAILED TO FIND KERNELBASE");

	return 0;
}

static int handle_rdtsc(struct kvm_vcpu *vcpu) {
	u64 offset, ratio, tsc;

	offset = vcpu->arch.l1_tsc_offset;
	ratio = vcpu->arch.l1_tsc_scaling_ratio;

	tsc = kvm_scale_tsc(vcpu, rdtsc(), ratio) + offset;
...
char* utf16toutf8(char* Destination, const u16* Source, u32 DestinationMaxLength)
{
	u32 i;
...
void kvm_virt_read_kernel(struct kvm_vcpu* vcpu, u64 addr, u32 len, void* buffer) {
	struct x86_exception e;
//	gpa_t x = kvm_mmu_gva_to_gpa_read(vcpu, addr, &e);
	gpa_t x = kvm_mmu_gva_to_gpa_system(vcpu, addr, &e);
	int status = kvm_read_guest(vcpu->kvm, x, buffer, len);
	if (status) {
		printk(KERN_ALERT "kvm_read_guest_failure[%x] 0x%llx -> 0x%llx", status, addr, x);
	}
}

static void dump_module_list(struct kvm_vcpu* vcpu, u64 kbase, u64 psloadedmodulelist) {
	...
	u64 count = 0;
	kvm_virt_read_kernel(vcpu, current_module, sizeof(u64), &current_module);
	if (!current_module) {
		INT_TRACE("CURRENT_MODULE INVALID");
	}

	printk(KERN_ALERT "current_module: 0x%llx\n", current_module);

	while ((current_module != psloadedmodulelist) && (count++ < 4096))
	{
		LDR_DATA_TABLE_ENTRY64 mod_info = {0};
		virt_read_kernel(vcpu, current_module, sizeof(LDR_DATA_TABLE_ENTRY64), &mod_info);
...
//		printk(KERN_ALERT "DRIVER[%s] : 0x%llx -> 0x%x\n", arr, mod_info.DllBase, mod_info.SizeOfImage);
		...
		virt_read_kernel(vcpu, current_module, sizeof(u64), &current_module);
		printk(KERN_ALERT "current_module[2]: 0x%llx\n", current_module);
		if (!current_module) {
			INT_TRACE("FAILED GETTING FLINK");
			break;
		}
...
static void vmx_cpuid_callback(struct kvm_vcpu *vcpu)
{
	u64 packed, out, kbase;
	u32 eax = kvm_rax_read(vcpu);
	u32 ecx = kvm_rcx_read(vcpu);
	u32 edx = kvm_rdx_read(vcpu);
	if (eax == 0x69420) {
		INT_TRACE("CPUID [EAX:0x69420]");
//		packed = ((u64)ecx) << 32 | edx;
//		virt_read(vcpu, packed, 0x4, &out);
//		printk(KERN_ALERT "READ[0x%llx] -> %i\n", packed, out);
//		out = 600;
//		virt_write(vcpu, packed, 0x4, &out);
//		printk(KERN_ALERT "WRITE[0x%llx] -> %i", packed, out);
//		virt_read(vcpu, packed, 0x4, &out);
//		printk(KERN_ALERT "READ[0x%llx] -> %i\n", packed, out);
	} else if (eax == 0x69421) {
		kbase = get_kernel_base(vcpu);

		u16 mz;
		virt_read_kernel(vcpu, kbase, sizeof(u16), &mz);
		printk(KERN_ALERT "mz-> %x\n", mz);

		packed = ((u64)ecx) << 32 | edx;
		printk(KERN_ALERT "kbase[0x%llx]\n", kbase);
		printk(KERN_ALERT "PsLoadedModuleList[0x%llx]\n", packed);
		dump_module_list(vcpu, kbase, kbase + packed);
	}
}
